...
async function startServer() {
    ...
    const broadcastEvent = (event, data = {}, userId) => {
        if (userId) {
    ...
    const normalizeNullableIdentifier = (val) => {
        if (val === undefined || val === null || val === '')
            return null;
        return val;
    };
    const normalizeOptionalValue = (val) => {
        if (val === undefined)
            return null;
        return val;
    };
    ...
    const logApiRequest = (route, payload) => {
        console.log(`[${route}] request`, payload);
    };
    const logApiQuery = (route, message, payload) => {
    ...
    const logApiError = (route, error, payload) => {
        if (payload !== undefined) {
    ...
    const authenticateToken = (req, res, next) => {
        ...
        jwt.verify(token, JWT_SECRET, (err, decoded) => {
            ...
            const user = db.prepare('SELECT id, username, full_name, role, team_id, is_active FROM users WHERE id = ?').get(decoded.id);
            if (!user)
                return res.sendStatus(403);
            if (!user.is_active)
    ...
    const checkPermission = (module, action) => {
      ...
      `).get(req.user.role, module);
            if (!permission)
                return res.status(403).json({ message: "Permission denied" });
            const hasPermission = (action === 'view' && permission.can_view) ||
    ...
    const createGlobalNotification = (title, message, type, reference_id) => {
    ...
    app.post("/api/login", (req, res) => {
        ...
        const user = db.prepare('SELECT * FROM users WHERE username = ?').get(toSql(username));
        ...
        const settings = db.prepare('SELECT * FROM user_settings WHERE user_id = ?').get(user.id) || { theme: 'light' };
        res.json({
            token,
    ...
    app.post("/api/auth/force-change-password", authenticateToken, (req, res) => {
    ...
    app.get("/api/me/permissions", authenticateToken, (req, res) => {
        ...
        const permissions = db.prepare('SELECT * FROM role_permissions WHERE role = ?').all(req.user.role);
        const permissionsMap = {};
        permissions.forEach(p => {
            permissionsMap[p.module] = {
                can_view: !!p.can_view,
    ...
    app.get("/api/permissions", authenticateToken, (req, res) => {
        const perms = db.prepare('SELECT * FROM role_permissions WHERE role = ? AND module = ?').get(req.user.role, 'settings');
    ...
    app.get("/api/company-info", (req, res) => {
        const settings = db.prepare("SELECT * FROM settings WHERE key IN ('company_name', 'company_logo', 'theme_color', 'theme')").all();
        const info = {};
        settings.forEach(s => {
            info[s.key] = s.value;
        });
    ...
    app.get("/api/global-settings", authenticateToken, (req, res) => {
    ...
    app.post("/api/global-settings", authenticateToken, (req, res) => {
        const perms = db.prepare('SELECT * FROM role_permissions WHERE role = ? AND module = ?').get(req.user.role, 'settings');
        ...
        const transaction = db.transaction(() => {
            if (company_name !== undefined && canEdit)
                stmt.run('company_name', toSql(company_name));
            if (company_logo !== undefined && canEdit)
                stmt.run('company_logo', toSql(company_logo));
            if (notifications_enabled !== undefined && canView)
                stmt.run('notifications_enabled', toSql(notifications_enabled.toString()));
            if (unlock_roles !== undefined && canEdit)
                stmt.run('unlock_roles', toSql(unlock_roles));
            if (theme_color !== undefined && canDelete)
                stmt.run('theme_color', toSql(theme_color));
            if (theme !== undefined && canDelete)
                stmt.run('theme', toSql(theme));
        });
        transaction();
        broadcastEvent('global-settings-updated');
        res.json({ success: true });
    });
    app.post("/api/permissions/bulk", authenticateToken, (req, res) => {
        const perms = db.prepare('SELECT * FROM role_permissions WHERE role = ? AND module = ?').get(req.user.role, 'settings');
    ...
    `);
        const transaction = db.transaction((perms) => {
            ...
            const userPerms = db.prepare('SELECT * FROM role_permissions WHERE role = ?').all(req.user.role);
            ...
            for (const p of perms) {
                ...
                if (req.user.role !== 'super_admin') {
                    ...
                    if (p.can_view && !uPerm.can_view)
                        p.can_view = 0;
                    if (p.can_create && !uPerm.can_create)
                        p.can_create = 0;
                    if (p.can_edit && !uPerm.can_edit)
                        p.can_edit = 0;
                    if (p.can_delete && !uPerm.can_delete)
                        p.can_delete = 0;
                }
    ...
    app.post("/api/notifications/:id/shown", authenticateToken, (req, res) => {
    ...
    const formatTeamDisplayName = (teamName, parentTeamName) => {
        if (!teamName)
            return '';
        if (!parentTeamName)
            return teamName;
    ...
    const isLowWeightQualityTeam = (teamName, parentTeamName) => {
    ...
    const buildDashboardEntryScope = (req, filters, options) => {
        let baseFrom = `
    ...
    const buildDailyProductionTrend = (rows, filters) => {
        if (!rows.length) {
            return [];
        }
        const toIsoDate = (value) => new Date(`${value}T00:00:00.000Z`);
        ...
        const rowMap = new Map(sortedRows.map((row) => [
            String(row.date),
            {
    ...
    const buildDashboardUserScope = (req, filters) => {
        let baseFrom = `
      FROM users u
    ...
    app.get("/api/dashboard/stats", authenticateToken, checkPermission('dashboard', 'view'), (req, res) => {
      ...
      `;
            ...
            const teamsCount = db.prepare(teamsCountQuery).get(...teamsCountParams);
            const usersCount = db.prepare(usersCountQuery).get(...usersCountParams);
    ...
    app.get("/api/dashboard/chart-data", authenticateToken, checkPermission('dashboard', 'view'), (req, res) => {
    ...
    // Production Entries
    app.get("/api/production", authenticateToken, checkPermission('production', 'view'), (req, res) => {
    ...
    `;
        ...
        LIMIT ? OFFSET ?
      `;
            const entries = db.prepare(dataQuery).all(...params, Number(limit), offset);
    ...
    app.post("/api/production", authenticateToken, checkPermission('production', 'create'), (req, res) => {
        ...
        try {
            ...
            if (!team_id && user_id) {
                const assignedUser = db.prepare('SELECT team_id FROM users WHERE id = ?').get(user_id);
            ...
            if (!team_id && (req.user.role === 'tl' || req.user.role === 'payment_posting')) {
                const tlTeam = db.prepare('SELECT id FROM teams WHERE team_leader_id = ? LIMIT 1').get(req.user.id);
                if (tlTeam)
                    team_id = tlTeam.id;
            }
            if (!team_id) {
            ...
            if (!client_name && team_id) {
                const team = db.prepare('SELECT client_name FROM teams WHERE id = ?').get(team_id);
                if (team)
      ...
      `).get(toSql(user_id), toSql(date), toSql(client_name));
            if (duplicate) {
          ...
          WHERE id = ?
        `).run(toSql(team_id), toSql(user_id), toSql(client_name), toSql(date), toSql(production_value), toSql(target_value), toSql(quality), toSql(quality_low), toSql(quality_high), toSql(notes), toSql(downtime), toSql(downtime_reason), toSql(sample_production), toSql(reporting_to), duplicate.id);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, toSql(team_id), toSql(user_id), toSql(client_name), toSql(date), toSql(production_value), toSql(target_value), toSql(quality), toSql(quality_low), toSql(quality_high), toSql(notes), toSql(downtime), toSql(downtime_reason), toSql(sample_production), toSql(reporting_to), req.user.id);
    ...
    app.put("/api/production/:id", authenticateToken, checkPermission('production', 'edit'), (req, res) => {
      ...
      `).get(toSql(checkUserId), toSql(checkDate), toSql(checkClientName), id);
            if (duplicate) {
        ...
        WHERE id = ?
      `).run(toSql(checkTeamId), toSql(checkUserId), toSql(checkDate), toSql(production_value), toSql(target_value), toSql(checkClientName), toSql(quality), toSql(quality_low), toSql(quality_high), toSql(notes), toSql(downtime), toSql(downtime_reason), toSql(sample_production), toSql(reporting_to), id);
    ...
    app.delete("/api/production/:id", authenticateToken, (req, res) => {
        ...
        if (req.user.role !== 'super_admin') {
            ...
            if (!isTLRole) {
                ...
                const perm = db.prepare('SELECT can_delete FROM role_permissions WHERE role = ? AND module = ?').get(req.user.role, 'production');
                if (!perm || !perm.can_delete) {
                    return res.status(403).json({ message: "Permission denied for delete on production" });
                }
            }
        }
        const entry = db.prepare('SELECT is_locked, team_id FROM production_entries WHERE id = ?').get(req.params.id);
        if (!entry) {
        ...
        if ((req.user.role === 'tl' || req.user.role === 'payment_posting') && req.user.role !== 'super_admin') {
            const team = db.prepare('SELECT team_leader_id FROM teams WHERE id = ?').get(entry.team_id);
    ...
    app.patch("/api/production/:id/toggle-lock", authenticateToken, (req, res) => {
        const entry = db.prepare('SELECT is_locked, created_by FROM production_entries WHERE id = ?').get(req.params.id);
        if (!entry) {
            return res.status(404).json({ message: "Production entry not found" });
        }
        ...
        if (isCurrentlyLocked) {
            const unlockSettings = db.prepare('SELECT value FROM settings WHERE key = ?').get('unlock_roles');
            ...
            const hasEditPerm = req.user.role === 'super_admin' || db.prepare('SELECT can_edit FROM role_permissions WHERE role = ? AND module = ?').get(req.user.role, 'production')?.can_edit === 1;
      ...
      `).get(req.params.id);
            if (fullEntry) {
                createGlobalNotification('Production Entry Added', `${fullEntry.user_name} added a production entry for ${fullEntry.team_name}.`, 'production', req.params.id);
                broadcastEvent('notifications-updated');
            }
        }
    ...
    // Teams
    app.get("/api/teams", authenticateToken, (req, res, next) => {
        if (req.user.role === 'member' || req.user.role === 'tl' || req.user.role === 'payment_posting')
            return next();
    ...
    app.post("/api/teams", authenticateToken, checkPermission('teams', 'create'), (req, res) => {
        ...
        try {
            ...
            if (parent_id) {
                const parent = db.prepare('SELECT name FROM teams WHERE id = ?').get(parent_id);
                if (parent) {
            ...
            const duplicate = db.prepare('SELECT id FROM teams WHERE LOWER(name) = LOWER(?)').get(finalName);
            if (duplicate) {
                return res.status(400).json({ success: false, message: parent_id ? "Sub-team already exists" : "Team already exists" });
            }
            const id = uuidv4();
            const transaction = db.transaction(() => {
                db.prepare('INSERT INTO teams (id, name, description, client_name, team_leader_id, parent_id) VALUES (?, ?, ?, ?, ?, ?)').run(id, toSql(finalName), toSql(description), toSql(client_name), toSql(team_leader_id), toSql(parent_id));
    ...
    app.put("/api/teams/:id", authenticateToken, checkPermission('teams', 'edit'), (req, res) => {
        ...
        try {
            ...
            const duplicate = db.prepare('SELECT id FROM teams WHERE LOWER(name) = LOWER(?) AND id != ?').get(finalName, req.params.id);
            if (duplicate) {
                return res.status(400).json({ success: false, message: parent_id ? "Sub-team already exists" : "Team already exists" });
            }
            const transaction = db.transaction(() => {
                db.prepare('UPDATE teams SET name = ?, description = ?, client_name = ?, team_leader_id = ?, parent_id = ? WHERE id = ?').run(toSql(finalName), toSql(description), toSql(client_name), toSql(team_leader_id), toSql(parent_id), req.params.id);
    ...
    app.delete("/api/teams/:id", authenticateToken, checkPermission('teams', 'delete'), (req, res) => {
        try {
            ...
            const userCount = db.prepare('SELECT COUNT(*) as count FROM users WHERE team_id = ?').get(req.params.id);
            ...
            const subTeamCount = db.prepare('SELECT COUNT(*) as count FROM teams WHERE parent_id = ?').get(req.params.id);
    ...
    `;
        ...
        const excludeRoles = req.query.exclude_roles ? req.query.exclude_roles.split(',') : [];
    ...
    app.get("/api/my-team", authenticateToken, (req, res) => {
        if (req.user.role !== 'tl' && req.user.role !== 'payment_posting') {
            return res.status(403).json({ message: "Forbidden" });
        }
        // Get all teams this TL leads (directly)
        const ledTeams = db.prepare('SELECT id FROM teams WHERE team_leader_id = ?').all(req.user.id);
        ...
        for (const teamId of ledTeamIds) {
            const subTeams = db.prepare('SELECT id FROM teams WHERE parent_id = ?').all(teamId);
    ...
    app.post("/api/users", authenticateToken, checkPermission('users', 'create'), (req, res) => {
        ...
        try {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run(id, toSql(String(username).trim()), toSql(String(full_name).trim()), toSql(email), toSql(role), finalTeamId, hashedPassword, 1);
                db.prepare(`
        ...
        `).run(id, 'light');
            ...
            transaction();
            if (finalTeamId) {
                const team = db.prepare('SELECT name, team_leader_id FROM teams WHERE id = ?').get(finalTeamId);
    ...
    app.put("/api/users/:id", authenticateToken, checkPermission('users', 'edit'), (req, res) => {
        ...
        try {
            ...
            const password = normalizeOptionalValue(req.body.password);
            const userToEdit = db.prepare('SELECT role, team_id FROM users WHERE id = ?').get(req.params.id);
            if (!userToEdit) {
    ...
    app.get("/api/clients", authenticateToken, (req, res) => {
        ...
        const clientsFromTable = db.prepare(query).all(...params);
        ...
        const clientsFromTeams = db.prepare(teamQuery).all(...teamParams);
    ...
    app.delete("/api/clients/:id", authenticateToken, checkPermission('production', 'delete'), (req, res) => {
        ...
        let client = db.prepare('SELECT name, team_id FROM clients WHERE id = ?').get(id);
        // If not found, check if it's a name (for legacy/aggregated entries)
        if (!client) {
            // Check if this name exists in clients or teams
            const nameExistsInClients = db.prepare('SELECT name FROM clients WHERE name = ?').get(id);
            const nameExistsInTeams = db.prepare('SELECT client_name FROM teams WHERE client_name = ?').get(id);
    ...
    // Targets
    app.get("/api/targets", authenticateToken, checkPermission('targets', 'view'), (req, res) => {
        let query = `
    ...
    app.post("/api/targets", authenticateToken, checkPermission('targets', 'create'), (req, res) => {
      ...
      `).get(toSql(team_id), toSql(user_id), toSql(effective_date));
        ...
        `).run(targetId, toSql(team_id), toSql(user_id), toSql(target_value), toSql(period), toSql(effective_date));
            ...
            const team = team_id ? db.prepare('SELECT name FROM teams WHERE id = ?').get(team_id) : null;
            const teamName = team ? team.name : 'Unknown Team';
            const action = existing ? 'Target Updated' : 'Target Created';
            const message = existing
                ? `${req.user.username} updated the target for ${teamName} (Effective: ${effective_date}).`
                : `${req.user.username} set a new target for ${teamName} (Effective: ${effective_date}).`;
