...
if (process.env.DATABASE_URL) {
  try {
    ...
    console.warn('Unable to parse DATABASE_URL for SSL detection:', error);
  }
}

const urlSsl = (parsedDatabaseUrl?.searchParams.get('ssl') || '').toLowerCase();
const urlSslMode = (parsedDatabaseUrl?.searchParams.get('sslmode') || '').toLowerCase();
const urlRequiresSsl = urlSsl === 'true' || urlSsl === '1' || SSL_MODE_VALUES.has(urlSslMode);
const isHostedProduction = process.env.RENDER === 'true' || process.env.NODE_ENV === 'production';
const hasRemoteDatabaseUrl = Boolean(
  parsedDatabaseUrl &&
  parsedDatabaseUrl.hostname &&
  !LOCAL_HOSTNAMES.has(parsedDatabaseUrl.hostname.toLowerCase())
);
const useSsl =
  process.env.PGSSL === 'true' ||
  SSL_MODE_VALUES.has(sslMode) ||
  urlRequiresSsl ||
  (isHostedProduction && hasRemoteDatabaseUrl);
